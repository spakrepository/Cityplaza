...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        if ($level == 0 && $drawPopup) {
        	if($category->getId() == 15 || $category->getId() == 38 || $category->getId() == 39){
        		$statUrl='#';
        	} else {
        		$statUrl = $this->getCategoryUrl($category);
        	}
            $htmlTop[] = '<a  class="level' . $level . $active . '" href="javascript:void(0);" rel="'.$statUrl.'">';
        } else {
